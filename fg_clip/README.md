...
| 3.4 难负例 | `fg_data/hard_negatives.py` | 颜色>动作>数量>材质 优先级，每正例 1~3 条 |
...
- 难负例用规则词表改写（方案中的工业版用 LLM 改写 + 语病校验）；
- 消融实验（去难负例/去区域损失）需自行跑三个配置对比；
- YOLO-World 对中文表达式的 grounding 能力有限，可在 config 中将表达式翻译为英文再检测（进阶）。
